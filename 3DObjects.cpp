...
void Camera::move(Vector2f mouse_pos)
{
...
float sphere::isRayIntersects(Vector3Df camera, Vector3Df RayDirection)
{
    ...
    float sum = s.x * RayDirection.x + s.y * RayDirection.y + s.z * RayDirection.z, sLenght = s.lenght();
...
float sphere::trayRay(Vector3Df camera, Vector3Df Ray, Vector3Df light, int lightPow)
{
    ...
    if (dist >= 0) {
        ...
        return 255 * -(to_light * normal) / (to_light.lenght() * normal.lenght());
    }
    else return -1000;
